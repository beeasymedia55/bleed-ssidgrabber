...
class ExtractSessionid:
    ...
    def choice(self):
        print(f'{self.purple}    __    __              __             _     __')
        print(f'{self.purple}   / /_  / /__  ___  ____/ /  __________(_)___/ /')
        print(f'{self.purple}  / __ \/ / _ \/ _ \/ __  /  / ___/ ___/ / __  / ')        
        print(f'{self.purple} / /_/ / /  __/  __/ /_/ /  (__  |__  ) / /_/ /  ')
        print(f'{self.purple}/_.___/_/\___/\___/\__,_/  /____/____/_/\__,_/   ')
        print(f'{self.purple}')
        print(f'[{self.purple}+{self.reset}] 1 - get ssid from text file ( account.txt )\n[{self.purple}+{self.reset}] 2 - Get ssid from email / password\n[{self.purple}+{self.reset}] option : ', end='')
        ...
        if self.number == 1:
            ...
            self.email = input()
            print(f'[{self.purple}+{self.reset}] Password : ', end='')
            self.password = input()
                ...
                try:
                    ...
                    try:
                        ...
                        if 'email' in self.login.text:
                            self.count +=1
                            print(f'[{self.purple}+{self.reset}] Successfully Login In {self.purple}{self.count}{self.reset} Account')
                            self.sessionid = self.login.json()['data']['session_key']
                            with open('sessions.txt', 'a') as self.saved:
                                self.saved.write(f'[+] Sessionid In {self.count} Account : {self.sessionid}\n')
                            time.sleep(sleep)
                        elif 'Incorrect account or password' in self.login.text:
                            self.count +=1
                            print(f'[{self.red}+{self.reset}] Password Error In {self.purple}{self.count}{self.reset} Account')
                            time.sleep(sleep)
                        elif 'Invalid Email address' or 'The account you entered does not exist.' in self.login.text:
                            self.count +=1
                            print(f'[{self.red}+{self.reset}] Email Error In {self.purple}{self.count}{self.reset} Account')
                            time.sleep(sleep)
                    ...
                    pass
            print(f'[{self.purple}+{self.reset}] Successfully For Check All Account\n[{self.purple}+{self.reset}] Press Enter To Exit')
            input()
            exit(0)
        except FileNotFoundError:
            print(f'[{self.purple}+{self.reset}] Successfully Created File ( account.txt )')
            self.count_account_number = 0
            self.create_file = open('account.txt', 'a')
            print(f'[{self.purple}+{self.reset}] How much do you want to enter an account : ', end='')
            self.count_account_input = int(input())
            for self.save_account in range(self.count_account_input):
                self.count_account_number +=1
                print(f'[{self.purple}+{self.reset}] Account {self.count_account_number}')
                print(f'[{self.purple}+{self.reset}] Email : ', end='')
                self.email = input()
                print(f'[{self.purple}+{self.reset}] Password : ', end='')
                self.password = input()
                ...
                os.system("cls")
                print(f'[{self.purple}+{self.reset}] 1 - Extract Sessionid From File ( account.txt )\n[{self.purple}+{self.reset}] 2 - Extract Sessionid From Input\n[{self.purple}+{self.reset}] Number : {self.number}')
                print(f'[{self.purple}+{self.reset}] Successfully Created File ( account.txt )')
                print(f'[{self.purple}+{self.reset}] How much do you want to enter an account : {self.count_account_input}')
                os.system("cls")
                print(f'[{self.purple}+{self.reset}] 1 - Extract Sessionid From File ( account.txt )\n[{self.purple}+{self.reset}] 2 - Extract Sessionid From Input\n[{self.purple}+{self.reset}] Number : 1')
                print(f'[{self.purple}+{self.reset}] Successfully Found File ( account.txt )')
                print(f'[{self.purple}+{self.reset}] Cooldown  : ', end='')
                self.sleep = int(input())
    ...
    def ExtractSessionidInput(self, email, password):
        try:
            ...
            if 'email' in self.login.text:
                self.sessionid = self.login.json()['data']['session_key']
                print(f'[{self.purple}+{self.reset}] Successfully Login\n[{self.purple}+{self.reset}] Sessionid : {self.sessionid}\n[{self.purple}+{self.reset}] Press Enter To Exit')
                with open('sessions.txt', 'a') as self.saved:
